...
const RESIZE_MODES = new Set<ResizeMode>(["none", "long-edge", "short-edge", "width", "height"]);
...
export function loadConvertOptions(data: unknown): ConvertOptions {
  ...
  return {
    ...
    size: typeof value.size === "number" && Number.isFinite(value.size) && value.size >= 1
      ? Math.round(value.size)
      : DEFAULT_CONVERT_OPTIONS.size,
    lossless: typeof value.lossless === "boolean" ? value.lossless : DEFAULT_CONVERT_OPTIONS.lossless,
    quality: typeof value.quality === "number" && Number.isFinite(value.quality)
